...
temp = data.frame(
    ...
    rating=movie_info[4],
    blurb=substr(blurb, 1, 50), #in case we captured too much
...
```{r scrapemany, comment=F, mesage=F, cache=T}
...
kable(one_month_reviews[,c(2:ncol(one_month_reviews))], "html") %>%
  kable_styling(full_width = T, position = "left")
```
...
go back far enough rated r for and very old one
